  ...
  <div class="h-full space-y-8 relative pb-20">
    ...
    <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4 border-b border-dark-800 pb-5">
      <div>
        <h2 class="text-3xl font-extrabold text-white tracking-tight flex items-center gap-3">
          ...
          <span class="px-3 py-1 rounded-full bg-dark-800 border border-dark-700 text-slate-400 text-sm font-medium">{{
            feedbacks.length }}</span>
        </h2>
        <p class="text-slate-400 mt-2 text-sm">Dengarkan audio keluhan pengguna, lihat prediksi model, dan simpan
          sebagai dataset tervalidasi.</p>
      </div>
    </div>
    ...
    <div class="bg-dark-900/40 backdrop-blur-xl border border-dark-800 rounded-3xl overflow-hidden shadow-2xl">
      ...
      <div v-else-if="feedbacks.length === 0" class="flex flex-col items-center justify-center py-20 px-4 text-center">
        ...
        <p class="text-slate-500 text-sm max-w-sm">Semua keluhan akurasi pelafalan telah ditinjau atau belum ada keluhan
          baru dari pengguna.</p>
      </div>

      <!-- Feedback list -->
      <div v-else class="divide-y divide-dark-800">
        <div v-for="fb in paginatedFeedbacks" :key="fb.feedback_id"
          class="p-6 hover:bg-dark-950/20 transition-all flex flex-col lg:flex-row gap-6 justify-between items-start">
          ...
          <div class="space-y-4 flex-1">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div class="bg-dark-950/30 border border-dark-800/50 p-3 rounded-xl">
                <span class="block text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Target
                  Latihan</span>
              ...
              <!-- Model Decision -->
              <div class="bg-dark-950/30 border border-dark-800/50 p-3 rounded-xl">
                <span class="block text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Hasil Model
                  CNN</span>
                <span class="text-xs font-semibold px-2 py-0.5 rounded-full inline-block mt-1"
                  :class="fb.is_correct ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'">
              ...
              <!-- Accuracy Score -->
              <div class="bg-dark-950/30 border border-dark-800/50 p-3 rounded-xl">
                <span class="block text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Akurasi
                  Prediksi</span>
              ...
              <div class="bg-dark-950/30 border border-dark-800/50 p-2.5 rounded-xl flex items-center justify-between">
                <div>
                  <span class="block text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Rekaman
                    User</span>
                  <span class="text-[10px] text-slate-400">Dengarkan audio</span>
                </div>
                <button @click="playAudio(fb.audio_url)"
                  ...
                  <svg v-if="currentPlayingUrl === fb.audio_url" xmlns="http://www.w3.org/2000/svg"
                    class="h-4 w-4 text-white" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div class="space-y-1.5" v-if="fb.top5_predictions && fb.top5_predictions.length > 0">
              <span class="text-[10px] text-slate-500 font-semibold uppercase tracking-wider block">Top 5 Prediksi
                CNN</span>
              <div class="flex flex-wrap gap-2">
                <div v-for="(pred, i) in fb.top5_predictions" :key="pred.label"
          ...
          <div
            class="w-full lg:w-auto shrink-0 flex flex-col justify-center items-end self-stretch lg:border-l border-dark-800 lg:pl-6 pt-4 lg:pt-0">
            <template v-if="fb.is_verified">
              <div
                class="w-full lg:w-auto px-6 py-2.5 bg-green-500/10 border border-green-500/20 text-green-400 font-bold rounded-xl text-sm flex items-center justify-center gap-2">
              ...
              <p class="text-[10px] text-slate-500 text-center lg:text-right mt-2 w-full">Dataset ini telah dimasukkan
                ke pool latih.</p>
            </template>
            <template v-else>
              <button @click="openVerifyModal(fb)"
              ...
              <p class="text-[10px] text-slate-500 text-center lg:text-right mt-2 w-full">Masukkan rekaman ke dataset
                pool latih.</p>
            </template>
          </div>
      ...
      <div class="px-6 pb-6 pt-6 border-t border-dark-800">
        <AppPagination 
      ...
      <div v-if="isVerifyModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div
          ...
          <p class="text-slate-400 text-sm leading-relaxed mb-5">
            Tentukan kebenaran lafal & label makhraj yang benar untuk rekaman ini agar dapat digunakan sebagai dataset
            latih
            model selanjutnya.
          </p>
          ...
          <form @submit.prevent="handleVerifySubmit" class="space-y-4">
            ...
            <div class="space-y-2">
              <label class="block text-sm font-medium text-slate-300">Label Makhraj Sebenarnya (Ground Truth)</label>
              <select v-model="verifiedLabel" required
                class="w-full bg-dark-950 border border-dark-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 transition-all cursor-pointer font-semibold">
              ...
              <p class="text-[10px] text-slate-500 mt-1">Ground truth label yang diucapkan oleh user dalam rekaman
                audio.
              </p>
            </div>
            ...
            <div class="space-y-2">
              <label class="block text-sm font-medium text-slate-300 mb-2">Penilaian Pelafalan</label>
              ...
              <p class="text-[10px] text-slate-500 mt-1">Tandai pelafalan makhraj dalam rekaman sudah benar secara
                tajwid
                atau salah.</p>
            </div>
            ...
            <!-- Notes -->
            <div class="space-y-2">
              <label class="block text-sm font-medium text-slate-300">Catatan Admin</label>
              <textarea v-model="adminNotes"
                placeholder="Tulis catatan opsional mengenai pelafalan makhraj atau kualitas audio..." rows="3"
            ...
            <div class="flex gap-3 justify-end pt-4">
              ...
              <button type="submit" :disabled="isSubmittingVerification"
                ...
                <span v-if="isSubmittingVerification"
