  ...
  <div class="h-full space-y-8 relative pb-20">
    ...
    <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4 border-b border-dark-800 pb-5">
      <div>
        ...
        <p class="text-slate-400 mt-1 text-sm">Kelola daftar huruf, ubah makhraj, atau tambahkan audio referensi Ustadz.</p>
      </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <h3 class="text-xl font-bold text-white flex items-center gap-2">
        ...
        <span class="px-2.5 py-0.5 rounded-full bg-dark-800 border border-dark-700 text-slate-400 text-xs font-medium">{{ filteredLetters.length }}</span>
      </h3>
      ...
      <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
        ...
        <!-- Add Button -->
        <button @click="openFormModal()" class="flex items-center justify-center gap-1.5 px-4 py-2 bg-primary-500 hover:bg-primary-600 text-white rounded-xl text-sm font-semibold transition-all shadow-lg shadow-primary-500/20 whitespace-nowrap">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div v-else-if="filteredLetters.length === 0" class="flex flex-col items-center justify-center py-20 bg-dark-900/40 border border-dark-800 rounded-3xl text-center">
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="border-b border-dark-800 bg-dark-950/40">
              ...
              <th class="p-5 text-xs font-semibold text-slate-400 uppercase tracking-wider">Cara Baca / Pronunciation</th>
          ...
          <tbody class="divide-y divide-dark-800/50">
            <tr v-for="letter in filteredLetters" :key="letter.id" class="hover:bg-dark-950/20 transition-colors group">
              <!-- Arabic Script -->
              ...
              <!-- Harakat -->
              <td class="p-5">
                <span class="px-2 py-1 rounded-md bg-dark-950 border border-dark-800 text-xs font-medium text-slate-400">
                  {{ letter.harakat }}
                </span>
              </td>
              <!-- Pronunciation -->
              ...
              <td class="p-5">
                <div class="flex items-center gap-2">
                  <!-- Play Button -->
                  <button 
                    v-if="letter.audio_url"
                    @click="playAudio(letter.audio_url)"
                  ...
                  >
                    <svg v-if="currentPlayingUrl === letter.audio_url" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-primary-400 animate-pulse" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  <!-- Upload Icon Button -->
                  <button 
                    @click="openAudioModal(letter)"
                    class="p-2 rounded-xl border border-dark-800 hover:border-purple-500/50 bg-dark-950/60 hover:bg-purple-500/10 text-slate-500 hover:text-purple-400 transition-all"
        ...
        <form @submit.prevent="handleFormSubmit" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="space-y-1.5">
              <label class="text-xs font-semibold text-slate-400">Nama Huruf</label>
              <input v-model="formBaseLetter" type="text" required placeholder="Contoh: Alif"
                class="w-full bg-dark-950 border border-dark-800 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/50" />
            </div>

            <!-- Harakat -->
            <div class="space-y-1.5">
              <label class="text-xs font-semibold text-slate-400">Harakat</label>
              <select v-model="formHarakat" required
                class="w-full bg-dark-950 border border-dark-800 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/50 cursor-pointer">
                <option value="Fathah">Fathah</option>
                <option value="Kasrah">Kasrah</option>
                <option value="Dhummah">Dhummah</option>
                <option value="Tanwin">Tanwin</option>
                <option value="Sukun">Sukun</option>
                <option value="Tasydid">Tasydid</option>
              </select>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <!-- Pronunciation -->
            <div class="space-y-1.5">
              <label class="text-xs font-semibold text-slate-400">Cara Baca (Transliterasi)</label>
            ...
            <!-- Arabic Script -->
            <div class="space-y-1.5">
              <label class="text-xs font-semibold text-slate-400">Simbol Arab</label>
              <input v-model="formArabicScript" type="text" required placeholder="Contoh: أ"
                class="w-full bg-dark-950 border border-dark-800 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/50 font-arabic text-lg" />
            </div>
          </div>

          <!-- Model Label -->
          <div class="space-y-1.5">
            <label class="text-xs font-semibold text-slate-400">Label Kelas Model (CNN / TFLite)</label>
            <input v-model="formModelLabel" type="text" required placeholder="Contoh: alif_fathah"
              class="w-full bg-dark-950 border border-dark-800 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/50 font-mono text-xs" />
            <p class="text-[10px] text-slate-500 mt-0.5">Label ini harus sesuai dengan output kelas dari model neural network Anda.</p>
          </div>
    ...
    <Teleport to="body">
      <div v-if="isAudioModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <p class="text-slate-400 text-sm leading-relaxed mb-4">
          Unggah contoh rekaman pelafalan Ustadz untuk huruf <strong class="text-white">{{ uploadLetterDisplay }}</strong>. Audio ini akan disimpan di Cloudinary dan menjadi referensi pembanding bagi pengguna.
        </p>

        <!-- Dropzone / Input -->
        <div class="border-2 border-dashed border-dark-800 hover:border-purple-500/50 rounded-2xl p-6 text-center cursor-pointer hover:bg-dark-950/20 transition-all flex flex-col items-center justify-center relative group">
          <input 
            type="file" 
            accept="audio/wav"
          ...
          />
          <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-slate-500 group-hover:text-purple-400 mb-2 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
